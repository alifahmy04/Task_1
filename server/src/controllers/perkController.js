...
export async function updatePerk(req, res, next) {
  try {
    // Validate only the fields present in req.body (partial validation)
    const perkUpdateSchema = perkSchema.fork(Object.keys(perkSchema.describe().keys), field => field.optional()).prefs({ noDefaults: true });
    const { value, error } = perkUpdateSchema.validate(req.body, { allowUnknown: false, stripUnknown: true });    if (error) return res.status(400).json({ message: error.message });

    // Find and update the perk by ID, returning the updated document
